...
export class Bank2Service {
  ...
  getTaxes() :Observable <any> {
    ...
    return this.http.get("http://127.0.0.1:8000/query/org2/all", httpOptions).pipe(map((response: Response) => {
  ...
  addTax(pan_number:string,rule_type:string,fy_year:string,interest_amount:string,other_amount:string,deduction_amount:string,gender:string,dob:string) {
    ...
    return this.http.post("http://127.0.0.1:8000/update/org2/"+pan_number,JSON.stringify({"rule_type":rule_type,"fy_year":fy_year,"interest_amount":interest_amount,"other_amount":other_amount,"deduction_amount":deduction_amount,"gender":gender,"dob":dob}), httpOptions).pipe(map((response: Response) => {
  ...
  createPan(pan_number:string,gender:string,dob: string) {
    ...
    return this.http.post("http://127.0.0.1:8000/pan/create/org2/",body, httpOptions).pipe(map((response: Response) => {
        console.log("Response Query: ",response);
  ...
  getHistory(pan_number:string) :Observable <any> {
    ...
    return this.http.get("http://127.0.0.1:8000/history/org2/"+pan_number, httpOptions).pipe(map((response: Response) => {
        console.log("Response: ",response);
        return response;
    }),tap(event => {}, this.handleError))
  ...
  ///////
//   for  26AS
  getdocument2(pan_number:string,path:string) :Observable <any> {
    var token = sessionStorage.getItem('token')
    console.log("Token for update rules",token)
