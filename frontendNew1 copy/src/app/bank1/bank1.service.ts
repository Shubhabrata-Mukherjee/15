...
export class Bank1Service {
 ...
 // getTaxes(): Observable<Tax[]> {
  ...
  getHistory(pan_number:string,org:string) :Observable <any> {
    var token = sessionStorage.getItem('token')
    console.log("Token for update rules",token)
    ...
    return this.http.get("http://127.0.0.1:8000/history/"+org+"/"+pan_number, httpOptions).pipe(map((response: Response) => {
        console.log("Response: ",response);
        return response;
    }),tap(event => {}, this.handleError))
