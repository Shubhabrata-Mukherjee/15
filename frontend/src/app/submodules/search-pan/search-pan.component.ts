...
export class SearchPanComponent implements OnInit {
  ...
  constructor(private bank: BankService,private itdservice : ItdService) { }
  ...
  viewTransactionBank(pan_number: string, fy_year: string) {
    ...
    this.bank.getUser_Detail(this.name,pan_number)
      .subscribe(
        res => {
  ...
  getHistoryBank() {
    //this.T_flag = true;
    this.bank.getHistory(this.name,this.show_pan_number).subscribe(
      res => {
        this.lt = []
  ...
  getCumulativeBank() {
    this.bank.getCumulative(this.show_pan_number,this.show_fy_year).subscribe(
      res=>{ 
        console.log(res)
  ...
  getTaxes() {
    ...
    this.bank.getTaxes(this.name).subscribe(
      res=> {
